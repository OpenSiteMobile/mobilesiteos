...
apps.mediaelement.player.notify_loading = null;
...
msos.onload_func_pre.push(
	function () {
		...
		apps.mediaelement.player.notify_loading = msos.notify.loading('Loading page and video player...', jQuery('title').text());
	}
);
		...
		jQuery('video').html5video(
			{
				success_function: function (plyr) {
					...
					apps.mediaelement.player.notify_loading.fade_out();
				}
			}
		);
	}
