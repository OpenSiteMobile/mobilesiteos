 ...
 *                          v1.3.15, v1.3.16, v1.4.1, v1.4.2, v1.4.5
...
msos.console.info('ng/v145_msos -> start.');
...
(function (window, document) {
    ...
    var angular = {},
        version = {
            ...
            minor: 4,
            dot: 5,
            codeName: 'permanent_internship_msos'
        },
    ...
    function urlResolve(url, note) {
        ...
        if (verbose) {
            msos_debug('ng - urlResolve -> done, output:', output);
        }
    ...
    function createInjector(modulesToLoad, strictDi) {
        ...
        function createInternalInjector(cache, factory) {
            ...
            function invoke(fn, self, locals, serviceName) {
                ...
                if (verbose === 'injector') {
                    msos_debug(temp_ci + temp_cii + ' - invoke ->  done' + (debug.length ? ': [' + debug.join(', ') + ']' : invoke_name));
                }
        ...
        msos_debug(temp_ci + ' ===> run instanceInjector.invoke.');
    ...
    $$CoreAnimateQueueProvider = function () {
        ...
        this.$get = ['$$AnimateRunner', '$rootScope', function ($$AnimateRunner, $rootScope) {
            ...
            function addRemoveClassesPostDigest(element, add, remove) {
                ...
                updateData = function (classes, value) {
                    var changed = false;

                    if (classes) {
                        classes = _.isString(classes)
                            ? classes.split(SPACE)
                            : _.isArray(classes) ? classes : [];
                        ...
                        forEach(
                            ...
                            function (className) {
                                if (className) {
                                    changed = true;
                                    data[className] = value;    // updates the data variable T/F
                                }
                            }
                        );
                    }
                ...
                classesAdded =   updateData(add,    true);
                classesRemoved = updateData(remove, false);
    ...
    $AnimateProvider = ['$provide', function ($provide) {
        ...
        this.register = function (name, factory) {
            ...
            msos_debug('ng - $AnimateProvider - register -> called, for: ' + (name || "'' (empty string)"));
    ...
    function Browser(window, document) {
        ...
        function fireUrlChange() {
            ...
            msos_debug(temp_br + ' - fireUrlChange -> start.');

            self_url = self.url();

            if (lastBrowserUrl === self_url && lastHistoryState === cachedState) {
                msos_debug(temp_br + ' - fireUrlChange -> no change!');
                return;
            }
            ...
            msos_debug(temp_br + ' - fireUrlChange -> done!');
        }
        ...
        this.$get = ['$$rAF', '$q',
            function ($$rAF, $q) {
                ...
                RAFPromise.prototype = {
                    ...
                    getPromise: function () {
                        if (!this.defer) {
                            this.defer = $q.defer('$CoreAnimateCssProvider');
                        }
    ...
    function $CompileProvider($provide, $$sanitizeUriProvider) {
        ...
        this.$get = [
            ...
            function addDirective(tDirectives, tAdded_directives, name, location, maxPriority, ignoreDirective, startAttrName, endAttrName) {
                ...
                if (vc) {
                    msos_debug(temp_cp + temp_ad + 'start, name: ' + name + ', location: ' + location, tAdded_directives);
                }

                if (name === ignoreDirective) {
                    if (vc) {
                        msos_debug(temp_cp + temp_ad + ' done, ignored name: ' + name + ', location: ' + location);
                    }
                    return null;
                }
                ...
                if (hasDirectives.hasOwnProperty(name)) {
                    ...
                    for (i = 0; i < directives.length; i += 1) {
                        ...
                        if ((maxPriority === undefined || maxPriority > directive.priority) && directive.restrict.indexOf(location) !== -1) {
                            ...
                            if (!tAdded_directives[directive.name]) {
                                ...
                                if (msos.config.debug) {
                                    msos.console.warn(temp_cp + temp_ad + 'already assigned directive: ' + directive.name + (directive.name !== name ? ', (ref. directive: ' + name + ')' : ''));
                                }
                            }
                        }
                    }
                    ...
                    if (vc) {
                        msos_debug(temp_cp + temp_ad + ' done, name: ' + name + ', location: ' + location);
                    }
                } else {
                    if (vc) {
            ...
            function addAttrInterpolateDirective(node, directives, value, name, allOrNothing) {
                ...
                if (vc) {
                    msos_debug(temp_cp + ' - $get - addAttrInterpolateDirective -> start.');
                }
                ...
                if (!interpolateFn) {
                    if (vc) {
                        msos_debug(temp_cp + ' - $get - addAttrInterpolateDirective ->  done, no interpolation function.');
                    }
                    return;
                }
                ...
                if (vc) {
                    msos_debug(temp_cp + ' - $get - addAttrInterpolateDirective ->  done!');
                }
            }
            ...
            function collectDirectives(node, node_directives, node_attrs, maxPriority, ignoreDirective) {
                ...
                if (verbose) {
                    msos_debug(temp_cp + temp_cd + 'start, for: ' + node_name);
                }
                ...
                switch (node_type) {
                    ...
                    case NODE_TYPE_ELEMENT:
                        ...
                        if (vc) {
                            msos_debug(temp_cp + temp_cd + 'type element, add name directive (no attributes): ' + node_norm_name);
                        }
                        ...
                        for (j = 0; j < jj; j += 1) {
                            ...
                            if (!at_ignore) {   // Don't bother if ngIgnore
                                ...
                                directive_norm_name = at_ng_name.replace(/(Start|End)$/, '');

                                if (directiveIsMultiElement(directive_norm_name)) {
                                    if (at_ng_name === directive_norm_name + 'Start') {
                            ...
                            if (_.isString(node_class_name) && node_class_name !== '') {
                                ...
                                while (match) {
                                    ...
                                    if (addDirective(node_directives, node_added_directives, at_ng_name_class, 'C', maxPriority, ignoreDirective)) {
                                        ...
                                        if (vc) {
                                            msos_debug(temp_cp + temp_cd + 'type element, add class directive: ' + at_ng_name_class);
                                        }
                        ...
                        try {
                            ...
                            if (match) {
                                ...
                                if (addDirective(node_directives, node_added_directives, at_ng_name_comment, 'M', maxPriority, ignoreDirective)) {
                                    ...
                                    if (vc) {
                                        msos_debug(temp_cp + temp_cd + 'type element, add comment directive.');
                                    }
            ...
            function ($httpBackend, $$cookieReader, $cacheFactory, $rootScope, $q, $injector) {
                ...
                function $http(requestConfig) {
                    ...
                    promise.success = function (fn) {
                        throw new Error('promise.success is gone!');
                    };

                    promise.error = function (fn) {
                        throw new Error('promise.error is gone!');
                    };
        ...
        this.$get = ['$rootScope', '$window', '$q', '$$q',
            ...
            function interval(fn_in, delay_in, count, invokeApply_in) {
                var temp_in = ' - interval',
                    ...
                    deferred = (skipApply ? $$q : $q).defer('$IntervalProvider_$get'),
                ...
                promise.then(null, null, (!hasParams) ? fn_in : function() {
            ...
            function create_eval_async_func(new_url) {
                ...
                $rootScope.$evalAsync(
                    function () {
                        ...
                        msos_debug(temp_lp + temp_re + 'called.');
 ...
 */
    ...
    function qFactory(nextTick, pvdr) {
        ...
        function processQueue(state) {
            ...
            msos_debug(temp_pq + 'start, state name: ' + state.name + ', state status: ' + state.status);
            ...
            for (i = 0; i < ii; i += 1) {
                ...
                try {
                    if (_.isFunction(fn)) {
                        ...
                        pq_debug += 'state status: 1';
                        deferred_pq.resolve(state.value);
                    } else {
                        pq_debug += 'for rejected';
        ...
        function scheduleProcessQueue(state) {
            var temp_sp = temp_qf + ' - scheduleProcessQueue',
                debug_out = ', state name: ' + state.name + ', state status: ' + state.status;
            ...
            nextTick(
                function () {
                    if (verbose) {
                        msos_debug(temp_sp + ' - (nextTick) callback -> start.');
                    }

                    processQueue(state);

                    if (verbose) {
                        msos_debug(temp_sp + ' - (nextTick) callback ->  done!');
                    }
                },
                state.name
            );
        ...
        Deferred.prototype = {
            ...
            resolve: function (val) {
                    ...
                    try {
                        ...
                        if (_.isFunction(then) && !self.resolved) {
                            ...
                            if (ps.pending.length) {
                                ...
                                r_dbug = 'set status: 1, value: ' + (_.isObject(val) ? 'object' : val);
                            }
                        }
                    } catch (e) {
                ...
                if (verbose) {
                    msos_debug(temp_qf + temp_r + ' done: ' + r_name + ', ' + r_dbug);
                }
            },
        ...
        this.$get = ['$rootScope', function ($rootScope) {
            return qFactory(
                    function (callback, state_name) {
                        if (verbose) {
                            msos_debug(temp_qp + ' - nextTick -> start, state name: ' + state_name);
                        }
                        ...
                        if (verbose) {
                            msos_debug(temp_qp + ' - nextTick ->  done, state name: ' + state_name);
                        }
                    },
                    '$q'
                );
    ...
    function $$QProvider() {
        ...
        this.$get = ['$browser', function ($browser) {
            return qFactory(
                    function (callback) {
                        if (verbose) {
                            msos_debug(temp_qqp + ' - nextTick -> start.');
                        }

                        $browser.defer(callback);

                        if (verbose) {
                            msos_debug(temp_qqp + ' - nextTick -> done!');
                        }
                    },
                    '$$q'
                );
            ...
            function ($rootScope,   $browser,   $q,   $$q,   $exceptionHandler) {
                ...
                function timeout(fn_to, delay_to, invokeApply_to) {
                    ...
                    var temp_to = ' - $get - timeout -> ',
                        ...
                        deferred = (skipApply ? $$q : $q).defer('$TimeoutProvider_$get'),
                        promise = deferred.promise,
                        timeoutId;

                    msos_debug(temp_tp + temp_to + 'start, name: ' + promise.$$state.name + ', delay: ' + delay_to);
    ...
    if (verbose) {
        msos_debug('ng - ngAttributeAliasDirectives -> created:', _.keys(ngAttributeAliasDirectives));
    }
    ...
    if (verbose) {
        msos_debug('ng - ngEventDirectives -> created:', _.keys(ngEventDirectives));
    }
...
msos.console.info('ng/v145_msos -> done!');
