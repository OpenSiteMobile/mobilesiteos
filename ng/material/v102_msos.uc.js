...
function MdConstantFactory() {
  ...
  webkit = /webkit/i.test(vendorPrefix);
...
function mdCompilerService($q, $http, $injector, $compile, $controller, $templateCache) {
  ...
  this.compile = function(options) {
    ...
    if (templateUrl) {
      ...
      resolve.$template = $q.when($q.defer('when_compile'), template);
    }

    // Wait for all the resolves to finish if they are promises
    return $q.all($q.defer('all_compile'), resolve).then(function(locals) {
  ...
  function InterimElementFactory($document, $q, $$q, $rootScope, $timeout, $rootElement, $animate,
    ...
    return function createInterimElementService() {
      ...
      function show(options) {
        ...
        var hideExisting = !options.skipHide && stack.length ? service.hide() : $q.when($q.defer('when_show'), true);
      ...
      function hide(reason, options) {
        if ( !stack.length ) return $q.when($q.defer('when_hide'), reason);
        options = options || {};

        if (options.closeAll) {
          var promise = $q.all($q.defer('all_hide'), stack.reverse().map(closeElement));
          stack = [];
          return promise;
        } else if (options.closeTo !== undefined) {
          return $q.all($q.defer('all_hide'), stack.splice(options.closeTo).map(closeElement));
        } else {
      ...
      function cancel(reason, options) {
        ...
        if ( !interim ) return $q.when($q.defer('when cancel'), reason);
      ...
      function destroy(target) {
        ...
        return interim ? interim.remove(SHOW_CANCELLED, false, {'$destroy':true}) : $q.when($q.defer('when_destroy'), SHOW_CANCELLED);
      }
      ...
      function InterimElement(options) {
        var self, element, showAction = $q.when($q.defer('when_InterimElement'), true);

        options = configureScopeAndTransitions(options);

        return self = {
          options : options,
          deferred: $q.defer('InterimElement'),
        ...
        function createAndTransitionIn() {
          return $q.when($q.defer('when_createAndTransitionIn'), function(resolve, reject){
        ...
        function transitionOutAndRemove(response, isCancelled, opts) {
          ...
          if ( !element ) return $q.when($q.defer('when_transitionOutAndRemove'), false);
          ...
          if ( options.$destroy === true ) {
            ...
            $q.when($q.defer('when_transitionOutAndRemove'), showAction)
                .finally(function() {
        ...
        function configureScopeAndTransitions(options) {
          ...
          return angular.extend({
            ...
            onRemove: function transitionOut(scope, element) {
              ...
              return element && $animate.leave(element) || $q.when($q.defer('when_configureScopeAndTransitions'));
            }
          }, options );
        ...
        function compileElement(options) {
          ...
          return compiled || $q.when($q.defer('when_compileElement'), function (resolve) {
              resolve({
                locals: {},
                link: function () {
        ...
        function showElement(element, options, controller) {
          ...
          return $q.when($q.defer('when_showElement'), function (resolve, reject) {
            try {
              // Start transitionIn
              $q.when($q.defer('when_showElement'), options.onShow(options.scope, element, options, controller))
                .then(function () {
        ...
        function hideElement(element, options) {
          ...
          return $qq.when($q.defer('when_hideElement'), function (resolve, reject) {
            try {
              // Start transitionIn
              var action = $$q.when($q.defer('when_hideElement'), options.onRemove(options.scope, element, options) || true );
  ...
  function ComponentRegistry($log, $q) {
    ...
    return self = {
      ...
      when : function(handle) {
        if ( isValidID(handle) ) {
          var deferred = $q.defer('ComponentRegistry');
        ...
        return $q.reject($q.defer('reject_ComponentRegistry'), "Invalid `md-component-id` value.");
      }
...
function AnimateDomUtils($mdUtil, $q, $timeout, $mdConstant, $animateCss) {
  ...
  return self = {
    ...
    waitTransitionEnd: function (element, opts) {
        ...
        return $q.when($q.defer('when_waitTransitionEnd'), function(resolve, reject){
          opts = opts || { };
...
(function() {
  ...
  var $$AnimateRunnerFactory = ['$q', '$$rAFMutex', function($q, $$rAFMutex) {
    ...
    AnimateRunner.prototype = {
      ...
      getPromise: function() {
        if (!this.promise) {
          var self = this;
          this.promise = $q.when($q.defer('when_$$AnimateRunnerFactory'), function (resolve, reject) {
  ...
  angular
    ...
    .factory('$animateCss', ['$window', '$$rAF', '$$AnimateRunner', '$$forceReflow', '$timeout',
                     function($window,   $$rAF,   $$AnimateRunner,   $$forceReflow,   $timeout) {
      ...
      function applyClasses(element, options) {
        if (options.addClass) {
          jQuery.addClass(element, options.addClass);
          options.addClass = null;
        }
        if (options.removeClass) {
          jQuery.removeClass(element, options.removeClass);
...
(function() {
  ...
  CalendarCtrl.prototype.changeDisplayDate = function(date) {
    ...
    if (!this.isInitialized) {
      ...
      return this.$q.when($q.defer('when_changeDisplayDate_build'));
    }

    // If trying to show an invalid date or a transition is in progress, do nothing.
    if (!this.dateUtil.isValidDate(date) || this.isMonthTransitionInProgress) {
      return this.$q.when($q.defer('when_changeDisplayDate_transition'));
    }
  ...
  CalendarCtrl.prototype.animateDateChange = function(date) {
    ...
    return this.$q.when($q.defer('when_animateDateChange'));
  };
...
function showInputMessages(element, $animateCss, $q) {
  ...
  return $q.all($q.defer('all_showInputMessages'), animators);
}
...
function hideInputMessages(element, $animateCss, $q) {
  ...
  return $q.all($q.defer('all_hideInputMessages'), animators);
}
  ...
  function selectDefaultOptions($mdSelect, $mdConstant, $mdUtil, $window, $q, $$rAF, $animateCss, $animate, $document) {
    ...
    function onShow(scope, element, opts) {
      ...
      function showDropDown(scope, element, opts) {
        ...
        return $q.when($q.defer('when_showDropDown'), function(resolve, reject) {
      ...
      function positionAndFocusMenu() {
        return $q.when($q.defer('when_positionAndFocusMenu'), function(resolve) {
          if (opts.isRemoved) return $q.reject($q.defer('reject_positionAndFocusMenu'), false);
      ...
      function watchAsyncLoad() {
        if (opts.loadingAsync && !opts.isRemoved) {
          ...
          $q.when($q.defer('when_watchAsyncLoad'), opts.loadingAsync)
            .then(function() {
...
function SidenavService($mdComponentRegistry, $q) {
  return function(handle) {
    ...
    return self = {
      ...
      // -----------------
      toggle: function() {
        return instance ? instance.toggle() : $q.reject($q.defer('reject_SidenavService_toggle'), errorMsg);
      },
      open: function() {
        return instance ? instance.open() : $q.reject($q.defer('reject_SidenavService_open'), errorMsg);
      },
      close: function() {
        return instance ? instance.close() : $q.reject($q.defer('reject_SidenavService_close'), errorMsg);
      },
      then : function( callbackFn ) {
        var promise = instance ? $q.when($q.defer('when_SidenavService'), instance) : waitForInstance();
...
function SidenavDirective($mdMedia, $mdUtil, $mdConstant, $mdTheming, $animate, $compile, $parse, $log, $q, $document) {
  ...
  function postLink(scope, element, attr, sidenavCtrl) {
    ...
    var promise = $q.when($q.defer('when_postLink'), true);
    ...
    function updateIsOpen(isOpen) {
      ...
      return promise = $q.all($q.defer('all_updateIsOpen'),
              [
    ...
    function toggleOpen( isOpen ) {
      if (scope.isOpen == isOpen ) {
        ...
        return $q.when($q.defer('when_toggleOpen'), true);

      } else {
        return $q.when($q.defer('when_toggleOpen'), function (resolve){
    ...
    function onKeyDown(ev) {
      ...
      return isEscape ? close(ev) : $q.when($q.defer('when_onKeyDown'), true);
    }
...
function SidenavController($scope, $element, $attrs, $mdComponentRegistry, $q) {
  ...
  self.$toggleOpen = function(value) { return $q.when($q.defer('when_SidenavController'), $scope.isOpen = value); };
...
function MdTooltipDirective($timeout, $window, $$rAF, $document, $mdUtil, $mdTheming, $rootElement,
  ...
  function postLink(scope, element, attr) {
    ...
    function hideTooltip() {
        ...
        $q.all($q.defer('all_hideTooltip'), promises)
          .then(function () {
...
function MdAutocompleteCtrl ($scope, $element, $mdUtil, $mdConstant, $mdTheming, $window,
  ...
  function getDisplayValue(item) {
    return $q.when($q.defer('when_getDisplayValue'), getItemText(item) || item);
...
"use strict";
 ...
 function MdIconService(config, $http, $q, $log, $templateCache) {
   ...
   function getIcon(id) {
     ...
     if ( iconCache[id]         ) return $q.when($q.defer('when_MdIconService'), iconCache[id].clone() );
   ...
   function loadFromIconSet(id) {
     ...
     function announceIdNotFound(id) {
      ...
      $log.warn(msg);
       ...
       return $q.reject($q.defer('reject_announceIdNotFound'), msg || id);
     }
   }
   ...
   function announceNotFound(err) {
     ...
     return $q.reject($q.defer('reject_announceNotFound'), msg);
   }
...
function MenuController($mdMenu, $attrs, $element, $scope, $mdUtil, $timeout, $rootScope, $q) {
  ...
  this.destroy = function() {
    return self.isOpen ? $mdMenu.destroy() : $q.when($q.defer('when_MenuController'), false);
  };
  ...
  function menuDefaultOptions($mdUtil, $mdTheming, $mdConstant, $document, $window, $q, $$rAF, $animateCss, $animate) {
    ...
    function onShow(scope, element, opts) {
      ...
      function showMenu() {
        ...
        return $q.when($q.defer('when_showMenu'), function(resolve) {
